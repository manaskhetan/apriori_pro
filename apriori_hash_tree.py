    ...
    def add_count(self, node, pick, rest, idx, k):
        ...
        if node.isleaf:
            ...
            for itemset in node.bucket:
                ...
                tmp_superset = [item for item in superset if item >=
                                itemset[0] and item <= itemset[-1]]
            ...
            for i in range(n_iter):
                ...
                if key in node.children:
                    self.add_count(
                        node.children[key], curr_pick, curr_rest, idx, k)
            # else:
            #     all_item = pick + rest
...
def get_new_candidates(all_frequent, k):
    if k == 1:
        new_candidates = list(
            map(lambda x: list(x), combinations(all_frequent[1][0], k+1)))
    elif k == 2:
        F_1 = all_frequent[1][0]
        new_candidates = [fk1_itemset+[f1_item] for fk1_itemset in all_frequent[2][0]
                          for f1_item in F_1 if f1_item not in fk1_itemset]
    else:
        Fk_1 = all_frequent[k][0]
        Fk_1_compare = set([tuple(itemset)
                            for itemset in Fk_1])  # create for hash comparing
        new_candidates = []
...
def apriori_student(df, dataset, te_array, min_support=0.001):
    ...
    while len(freq_itemsets) > 0:
        candidates = [tuple(candidate)
                      for candidate in get_new_candidates(all_frequent, k)]
        ...
        freq_itemsets, count_itemsets = htree.freq_itemsets(
            htree.root, support, [], [])
        ...
        if freq_itemsets and count_itemsets:
            sort_itemsets = sorted(
                zip(freq_itemsets, count_itemsets), key=lambda x: x[0])
            freq_itemsets, count_itemsets = (
                list(x) for x in zip(*sort_itemsets))
...
if __name__ == '__main__':
    # ======== Code for Test =========
    candidate = [(1, 4, 5), (1, 2, 4), (4, 5, 7), (1, 2, 5), (4, 5, 8), (1, 5, 9), (1, 3, 6),
